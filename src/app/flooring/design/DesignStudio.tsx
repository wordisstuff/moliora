...
const toneBackground: Record<string, string> = {
    Light: 'linear-gradient(135deg,#d8c7aa,#b99d74)', Natural: 'linear-gradient(135deg,#b79a72,#8e704d)', Warm: 'linear-gradient(135deg,#a87951,#704b30)', Dark: 'linear-gradient(135deg,#655448,#332b27)',
};
    ...
    return (
        <div className="grid gap-6 lg:grid-cols-[.72fr_1.28fr]">
            <div className="space-y-6 border border-white/10 bg-white/[.025] p-6">
                ...
                <p className="text-xs leading-5 text-white/40">Preview is conceptual, not an exact color match. Final material should be approved from a physical sample.</p>
            </div>
            ...
            <div className="flex min-h-[500px] flex-col border border-white/10 bg-white/[.02] p-4 sm:p-6">
                <div className="relative min-h-[390px] flex-1 overflow-hidden border border-white/10 bg-[#d8d4cd]">
                    {mode === 'photo' && photoUrl ? <><img src={photoUrl} alt="Your room preview" className="absolute inset-0 h-full w-full object-cover" /><div className="absolute inset-x-[5%] bottom-0 h-[43%] origin-bottom opacity-70 [clip-path:polygon(9%_0,91%_0,100%_100%,0_100%)]" style={{ background: floorBackground }}><div className="absolute inset-0 opacity-60" style={{ backgroundImage: plankPattern }} /></div><div className="absolute bottom-3 left-3 right-3 bg-black/70 p-3 text-xs leading-5 text-white/75">Beta overlay: the floor area is approximate. Automatic floor detection/masking is the next phase.</div></> : <><div className="absolute inset-x-0 top-0 h-[58%] bg-[linear-gradient(#dad7d0,#c6c2bb)]" /><div className="absolute left-1/2 top-[9%] h-[30%] w-[28%] -translate-x-1/2 border-[7px] border-white/75 bg-[linear-gradient(#9db5c3,#dce7eb)] shadow-lg"><div className="absolute left-1/2 top-0 h-full w-px bg-white/60" /><div className="absolute left-0 top-1/2 h-px w-full bg-white/60" /></div><div className="absolute inset-x-0 bottom-0 h-[43%] origin-bottom [clip-path:polygon(7%_0,93%_0,100%_100%,0_100%)]" style={{ background: floorBackground }}><div className="absolute inset-0 opacity-55" style={{ backgroundImage: plankPattern }} /></div><RoomFurniture room={room} /><div className="absolute left-3 top-3 bg-black/55 px-3 py-2 text-[10px] font-bold uppercase tracking-[.18em] text-white/75">Moliora room template</div></>}
                </div>
